...
def _get_first_nonempty(config, sections, option):
    for section in sections:
        if section in config and option in config[section]:
            value = str(config[section][option]).strip()
            if value:
                return value
    return None


def _build_mysql_sqlalchemy_uri(host, username, password, database, port):
...
def get_usage_stats_db_uri(config):
    # Preferred canonical form: component-based config.
    ...
    if host and username is not None and password is not None and database:
        return _build_mysql_sqlalchemy_uri(host, username, password, database, port)

    # Backward compatibility: URI-based config in [USAGE_STATS_DB] or legacy [DB].
    legacy_uri = _get_first_nonempty(
        config,
        ("USAGE_STATS_DB", "DB"),
...
class UsageStatsDatabaseHelper:
    ...
    def __init__(self, config):
        self.config = config
        self.engine = create_engine(get_usage_stats_db_uri(config))
